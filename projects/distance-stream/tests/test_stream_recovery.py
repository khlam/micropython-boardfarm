...
behaviors of the read-error branch plus the out-of-range gap branch:
  ...
  2. median_buf and ema are cleared so the next good sample seeds fresh;
  ...
  5. out-of-range readings emit `distance_mm: null` and clear filter state
     so the next in-range sample re-seeds rather than blending across the gap.
"""
...
def test_read_err_resets_filter_state(main_ns):
    # Three good 100s seed median_buf and the EMA at 100. After the OSError
    # both are cleared, so the next in-range sample (500) reseeds the EMA
    # from itself — no blending across the fault.
...
def test_out_of_range_emits_null_and_clears_state(main_ns):
    # 100 seeds the EMA at 100; 8190 (== OUT_OF_RANGE_MM) emits null and
    # clears state; 200 then reseeds from itself, proving the gap broke the
    # filter chain instead of being blended in.
