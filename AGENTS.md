...
| UART reports (internal) | `firmware-packages/uart_reports/uart_reports/` | `stream.py` — `ReportStream`, the IRQ-driven reader every radar driver subclasses (framing, resync, newest-report selection, `wait_ready`/`read_latest`/`close`) + `DeviceNotFoundError`; consumed only by drivers, never by projects |
| LD2450 radar | `projects/ld2450/`, `firmware-packages/ld2450/` | `LD2450(bus_id=, tx=, rx=)` — framing constants and the three-slot decoder over `ReportStream`; project firmware streams target JSON to the dashboard |
| LD2420 radar | `firmware-packages/ld2420/` | `LD2420(bus_id=, tx=, rx=)` — same, but `_prepare()` commands energy mode at startup and it reports range only (`Target(distance_mm)`) |
...
| Matter occupancy sensor | `projects/matter-radar-sensor/` | `firmware/radar.py` — detects whichever radar is wired to the shared UART and normalizes its reports; `firmware/main.py` — translates those target reports into a read-only Occupancy Sensor endpoint, with a second virtual Dimmable Light endpoint whose level maps linearly to the 0–10 minute hold after the last target; also serves its own dashboard over `httpd` once Matter has a network address |
