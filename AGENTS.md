...
| Matter occupancy sensor | `projects/matter-radar-sensor/` | `firmware/main.py` — translates LD2450 target reports into a read-only Occupancy Sensor endpoint, with a second virtual Dimmable Light endpoint whose level maps linearly to the 0–10 minute hold after the last target; also serves its own dashboard over `httpd` once Matter has a network address |
