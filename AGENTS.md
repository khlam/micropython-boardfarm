...
| LD2420 radar | `firmware-packages/ld2420/` | `LD2420(bus_id=, tx=, rx=)` — same lifecycle contract as `LD2450`, but commands energy mode at startup and reports range only (`Target(distance_mm)`) |
