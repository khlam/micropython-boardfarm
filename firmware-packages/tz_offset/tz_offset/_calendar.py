"""Pure-integer Gregorian calendar primitives for the tz_offset package.

Kept separate from the package ``__init__`` so the longitude heuristic and the
calendar math stay independently readable. Everything here is integer-only with
no I/O, so it runs identically on the MCU and under host CPython pytest.
"""
...
def weekday(year: int, month: int, day: int) -> int:
    ...
    sunday_anchored = (y + y // 4 - y // 100 + y // 400 + t[month - 1] + day) % 7
...
def utc_to_local_seconds(
    ...
    The offset is an arbitrary signed second count, so this also covers sub-hour
    zones (India +5:30, Nepal +5:45) rather than whole hours only. Done by
    converting to a day count plus seconds-of-day, adding the offset, and
    converting back with one ``divmod`` — no nested carry loops, and naturally
    correct across day/month/year/leap boundaries for offsets up to the ±14 h
    real-world range.
