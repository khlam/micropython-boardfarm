...
`chip` swaps os.uname so the strip dispatcher picks each backend in turn.
`_reset_stubs` clears recorded pin/NeoPixel state and the cached `ws2812b.*`
imports between tests so chip-dispatch reruns from a clean slate.
"""
