...
class VL53L0X:
    def __init__(self, *, sda, scl, address=0x29, skip_spad_info=True, interrupt_status_mask=0xFF):
        ...
        # and the SPAD-map rewrite. The procedure hangs on ESP32-S3 — the
        # chip ACKs the trigger write but never signals "done" — and
        # leaves the chip half-configured. Skipping uses the factory SPAD
        # map; ranging works, accuracy past ~1 m may degrade slightly.
        self.skip_spad_info = skip_spad_info
        # Mask applied to _RESULT_INTERRUPT_STATUS (0x13) when polling for
        # "measurement done". RP2040/RP2350 use 0x07 (bits 0-2 = standard
        # "new sample ready" per the ST datasheet). ESP32-S3 signals done
        # via bit 6 instead and bits 0-2 never set, so the ESP32 backend
    ...
    def _registers(self, register, values=None, struct="B"):
        # Unconditional retry on transient I²C errors. ESP32-S3 needs this
        # because the chip briefly NACKs its own address byte mid-init under
        # bit-banged SoftI2C timing. RP2040/RP2350 also need it: hardware I²C
        # times out (ETIMEDOUT) when the VL53L0X clock-stretches during init,
        # depending on pullup values and breakout characteristics. 50 × 10 ms
        # is enough slack for every busy window observed, and the retry never
        # fires on a fully reliable bus so it's free insurance.
        for attempt in range(50):
            try:
                if values is None:
                    size = ustruct.calcsize(struct)
                    data = self.i2c.readfrom_mem(self.address, register, size)
                    return ustruct.unpack(struct, data)
    ...
    def init(self, power2v8=True):
        ...
        # else: keep the chip's factory SPAD map — modifying it with a
        # guessed count/aperture leaves the chip with no usable SPADs.
    ...
    def _spad_info(self):
        ...
        # Bit 3 (mask 0x08). The ST/Pololu/Adafruit reference drivers use
        # bit 2 here, but this MicroPython community port has always used
        # bit 3 — RP2040 init only completes with bit 3; switching to bit 2
        # leaves the chip in a state where the 0x83 status poll a few lines
        # below never returns non-zero and _spad_info() raises TimeoutError.
        # ESP32-S3 sidesteps this entirely via skip_spad_info=True, so the
        # community-port behaviour is the right default.
        ...
        for timeout in range(_IO_TIMEOUT):
            ...
            raise TimeoutError("spad_info 0x83 poll")
        self._config(
            (0x83, 0x01),
        )
    ...
    def _calibrate(self, vhv_init_byte):
        ...
        # Mask gated by self.interrupt_status_mask: RP uses 0x07 (bits 0-2),
        # ESP32-S3 passes 0xFF (signals done via bit 6).
        for timeout in range(_IO_TIMEOUT):
            if self._register(_RESULT_INTERRUPT_STATUS) & self.interrupt_status_mask:
                break
            utime.sleep_ms(1)
        else:
            raise TimeoutError("calibrate interrupt_status poll vhv=0x%02x" % vhv_init_byte)
        self._register(_INTERRUPT_CLEAR, 0x01)
    ...
    def read(self):
        if not self._started:
            ...
            for timeout in range(_IO_TIMEOUT):
                ...
                raise TimeoutError()
        # Mask gated by self.interrupt_status_mask. RP2040/RP2350 use 0x07
        # (bits 0-2 = ST datasheet "new sample ready"); a wider mask exits
        # the loop on a spurious bit and returns range register before the
        # measurement completes (value reads 0). ESP32-S3 passes 0xFF
        # because bits 0-2 never set there — done is signaled via bit 6.
        for timeout in range(_IO_TIMEOUT):
            if self._register(_RESULT_INTERRUPT_STATUS) & self.interrupt_status_mask:
                break
            utime.sleep_ms(1)
    ...
    def set_Vcsel_pulse_period(self, type, period_pclks):
        ...
        if type == self.vcsel_period_type[0]:
            ...
            self._register(
                PRE_RANGE_CONFIG_TIMEOUT_MACROP_HI, self.encode_timeout(new_pre_range_timeout_mclks)
            )
    ...
    def get_sequence_step_timeouts(self):
        ...
        self.timeouts["msrc_dss_tcc_us"] = self.timeout_Mclks_to_microseconds(
            self.timeouts["msrc_dss_tcc_mclks"], self.timeouts["pre_range_vcsel_period_pclks"]
        )
        ...
        self.timeouts["pre_range_us"] = self.timeout_Mclks_to_microseconds(
            self.timeouts["pre_range_mclks"], self.timeouts["pre_range_vcsel_period_pclks"]
        )
        ...
        self.timeouts["final_range_us"] = self.timeout_Mclks_to_microseconds(
            self.timeouts["final_range_mclks"], self.timeouts["final_range_vcsel_period_pclks"]
        )
    ...
    def set_measurement_timing_budget(self, budget_us):
        ...
        if self.enables["final_range"]:
            ...
            self._register(
                FINAL_RANGE_CONFIG_TIMEOUT_MACROP_HI, self.encode_timeout(final_range_timeout_mclks)
            )
    ...
    def perform_single_ref_calibration(self, vhv_init_byte):
        ...
        while (self._register(RESULT_INTERRUPT_STATUS) & 0x07) == 0:
            if utime.ticks_diff(utime.ticks_ms(), chrono_start) > _IO_TIMEOUT:
                return False
