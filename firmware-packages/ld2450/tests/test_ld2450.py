...
``uart_reports`` and are tested there; these cover the LD2450's own UART
...
from ld2450 import LD2450, Target
...
def test_reports_read_end_to_end_decode_into_targets(radar, build_report):
    machine.feed_uart_bytes(build_report((1, 2, 0, 3)))
    asyncio.run(radar.wait_ready())

    assert asyncio.run(radar.read_latest()) == (Target(1, 1, 2, 0, 3),)
