...
from ld2420 import DeviceNotFoundError
from ld2420.ld2420 import (
    _ACK_BUFFER_LIMIT,
    _COMMAND_FOOTER,
    _COMMAND_HEADER,
    _CONFIGURATION,
...
def test_startup_sends_the_energy_mode_sequence(
    ...
    start_ready(radar, build_report(), configuration_acks)
...
def test_rejected_command_raises_and_closes(radar, build_ack):
...
def test_missing_ack_raises_and_closes(radar):
    with pytest.raises(DeviceNotFoundError, match="no LD2420 ACK for command 0x00ff"):
        asyncio.run(radar.wait_ready())
    assert machine.uart_constructions[0].deinitialized is True


def test_ack_arriving_after_the_wait_begins_is_accepted(radar, build_ack):
...
def test_ack_for_another_command_is_skipped(radar, build_report, build_ack, start_ready):
    ...
    start_ready(radar, build_report(distance_cm=30), acks)

    assert len(machine.uart_constructions[0].writes) == 3


def test_ack_with_a_bad_footer_is_not_accepted(radar, build_ack):
...
def test_ack_buffer_is_trimmed_to_its_limit(radar):
    """Unanswered chatter cannot grow the shared heap without bound."""
    machine.queue_uart_replies([bytes(_ACK_BUFFER_LIMIT + 44)])

    with pytest.raises(DeviceNotFoundError, match="no LD2420 ACK"):
        asyncio.run(radar.wait_ready())
    assert len(radar._ack_buffer) == _ACK_BUFFER_LIMIT


def test_write_failure_closes_and_reraises(radar):
