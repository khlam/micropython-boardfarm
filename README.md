...
### ESP32-S3-Zero
1. Put the board in bootloader mode: hold **BOOT** and tap **RESET** OR hold **BOOT** and connect USB. Confirm it appears as `/dev/ttyACM0` on the host — the `esp32` service fails fast if the device node is missing. **On macOS, do the [serial bridge setup](#macos-serial-bridge) first** — Docker can't see USB devices there.
2. Build and flash in one step:
   ```
   docker compose run --rm --build esp32
   ```
   This builds `outputs/app.esp32-s3.bin` and runs `esptool.py` against `$SERIAL_PORT` (default `/dev/ttyACM0`).
3. Power-cycle to boot into the new firmware.


## macOS: serial bridge
Docker Desktop runs containers inside a Linux VM, and neither it nor Apple's Virtualization.framework can pass a USB-serial device into that VM — so the `/dev:/dev` mount the `viz` and `esp32` services rely on sees nothing on macOS. [`tools/serial-bridge.sh`](tools/serial-bridge.sh) works around this with a full-duplex serial↔TCP relay built from stock tools (`stty`/`cat`/`nc`) — nothing is installed on the host. Set it up **once**, then every project's commands match Linux.
...
3. Run the dashboard or flash exactly as on Linux — both read `SERIAL_PORT`:
   ...
   docker compose run --rm --build esp32  # builds + flashes over the bridge
   ```
   The bridge carries no DTR/RTS reset line, so flashing requires the board to **already** be in bootloader mode (hold **BOOT**, tap **RESET**); the `esp32` stage passes `--before/--after no_reset` automatically for `socket://` ports.

> RP2040 / RP2350 firmware flashes by drag-copying a UF2 to the mounted bootloader drive, so it needs no serial bridge — only the `esp32` flash and the `viz` dashboard use the serial port.
