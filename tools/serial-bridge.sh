...
# `docker compose run esp32-flash` works the same as on Linux.
#
...
# VM. So the `- /dev:/dev` mount the esp32-flash service relies on is a no-op on
...
#
# The relay is full-duplex: esptool both reads and writes, so a
